...
BOOST_AUTO_TEST_CASE(test_listC_Var_Length)
{
  ...
  auto b = Main([a = a.as_ref()](dtime t0) { return Length(a); });
...
BOOST_AUTO_TEST_CASE(test_listA_Var_assign_Get)
{
  ...
  auto x = Main([lst = lst.as_ref(), idx = idx.as_ref(), vd = vd.as_ref()](
                  dtime t0) { return FromMaybe(Get(lst, idx), vd); });
...
BOOST_AUTO_TEST_CASE(test_listC_Var_assign_Get)
{
  ...
  auto x = Main([lst = lst.as_ref(), idx = idx.as_ref()](dtime t0) {
    return FromMaybe(Get(lst, idx), "default");
  });
...
BOOST_AUTO_TEST_CASE(test_listC_Var_assign_Get_via_subscript_operator)
{
  ...
  auto x = Main([lst = lst.as_ref(), idx = idx.as_ref()](dtime t0) {
    return FromMaybe(lst[idx], "default");
  });
...
BOOST_AUTO_TEST_CASE(test_ListC_Append)
{
  Engine engine;

  auto xs = Var(make_listC(1, 2, 3));

  auto ys = Append(xs, 11);

  auto f = Main(ys);

  BOOST_CHECK_EQUAL(core::to_string(*f), "list(1 2 3 11)");

  xs = make_listC(3, 4);

  BOOST_CHECK_EQUAL(core::to_string(*f), "list(3 4 11)");
}
...
BOOST_AUTO_TEST_CASE(test_ListC_append_ref_mem_func)
{
  Engine engine;

  auto xs = Var(make_listC(1, 2, 3));

  auto ys = xs.append(11);
...
BOOST_AUTO_TEST_CASE(test_listC_State)
{
  ...
  auto xs = Main([x = x.as_ref()](dtime t0) {
    auto xp = Prev(0, x, t0);
...
BOOST_AUTO_TEST_CASE(test_listC_Var_ToString)
{
  ...
  auto f = Main([xs = xs.as_ref()](dtime t0) { return ToString(xs); });
...
BOOST_AUTO_TEST_CASE(test_ListC_ToString_with_delimiter)
{
  ...
  auto f = Main([ys, delimiter = delimiter.as_ref()](dtime t0) {
    return ToString(ys, delimiter);
  });
