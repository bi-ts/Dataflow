...
namespace internal
{
DATAFLOW___EXPORT void update_node_recursion(node_id id);

template <typename T> class node_recursion final : public node_t<T>
{
...
public:
  static ref create(const ref& sp, const ref& s0, const ref& s)
  {
    ...
    return nodes_factory::create_conditional<node_recursion<T>>(
      &args[0], args.size(), node_flags::eager);
  }

private:
  explicit node_recursion()
  {
  }
